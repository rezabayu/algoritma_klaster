...
OUT_BASE = "outputs/cnn_sequence14_random_oversampling"
...
# Target rasio positif terhadap negatif pada TRAIN setelah oversampling.
# original = tanpa oversampling.
# os_pos_1_10 berarti positif dibuat sekitar 1/10 dari jumlah negatif.
# os_pos_1_5  berarti positif dibuat sekitar 1/5 dari jumlah negatif.
# os_pos_1_3  berarti positif dibuat sekitar 1/3 dari jumlah negatif.
# os_pos_1_1  berarti positif dibuat sama banyak dengan negatif.
OVERSAMPLING_SCENARIOS = {
    "original": None,
    "os_pos_1_10": 10,
    "os_pos_1_5": 5,
    "os_pos_1_3": 3,
...
def build_sequence_aux_cnn(seq_shape, aux_shape):
    ...
    model = models.Model(
        ...
        outputs=output,
        name="cnn_sequence14_aux_random_oversampling"
    )
...
def make_oversampled_train_indices(df_model, target_ratio_neg_per_pos, seed=42):
    """
    Random oversampling hanya pada data train.

    target_ratio_neg_per_pos:
    - None  : original, tanpa oversampling
    - 10    : positif dibuat kira-kira 1/10 dari negatif
    - 5     : positif dibuat kira-kira 1/5 dari negatif
    - 3     : positif dibuat kira-kira 1/3 dari negatif
    - 1     : positif dibuat sama dengan negatif

    Semua negatif train dipertahankan.
    Positif train diduplikasi secara acak sampai mencapai target.
    """

    df_train = df_model[df_model["Split"] == "train"].copy()

    pos_idx = df_train[df_train["Label_Lebat_50"] == 1].index.to_numpy().copy()
    neg_idx = df_train[df_train["Label_Lebat_50"] == 0].index.to_numpy().copy()

    if target_ratio_neg_per_pos is None:
        final_idx = df_train.index.to_numpy().copy()
        rng = np.random.default_rng(seed)
        rng.shuffle(final_idx)
        return final_idx

    n_neg = len(neg_idx)
    n_pos_original = len(pos_idx)

    target_n_pos = int(np.ceil(n_neg / target_ratio_neg_per_pos))
    target_n_pos = max(target_n_pos, n_pos_original)

    rng = np.random.default_rng(seed)

    sampled_pos_idx = rng.choice(
        pos_idx,
        size=target_n_pos,
        replace=True
    )

    final_idx = np.concatenate([neg_idx, sampled_pos_idx]).copy()
    rng.shuffle(final_idx)
...
dist_table.to_csv(
    f"{OUT_TABLES}/tabel_oversampling_01_distribusi_label_asli.csv",
    index=False
)

df_model.to_csv(
    f"{OUT_PROCESSED}/dataset_cnn_sequence14_random_oversampling_metadata.csv",
    index=False
)


# ============================================================
# 4. SPLIT INDEX DAN NORMALISASI
# ============================================================

idx_train_full = df_model.index[df_model["Split"] == "train"].to_numpy()
idx_val = df_model.index[df_model["Split"] == "validation"].to_numpy()
idx_test = df_model.index[df_model["Split"] == "test"].to_numpy()

# Scaler dipelajari dari seluruh train asli.
seq_scaler = StandardScaler()
seq_scaler.fit(X_seq[idx_train_full].reshape(-1, 1))

X_seq_scaled = seq_scaler.transform(X_seq.reshape(-1, 1)).reshape(X_seq.shape)

aux_scaler = StandardScaler()
aux_scaler.fit(X_aux[idx_train_full])
...
baseline_table.to_csv(
    f"{OUT_TABLES}/tabel_oversampling_02_evaluasi_baseline.csv",
    index=False
)


# ============================================================
# 6. TRAIN CNN DENGAN SKENARIO RANDOM OVERSAMPLING
# ============================================================
...
for scenario_name, ratio in OVERSAMPLING_SCENARIOS.items():
    print(f"\nTraining skenario: {scenario_name}")

    idx_train_scenario = make_oversampled_train_indices(
        df_model=df_model,
        target_ratio_neg_per_pos=ratio,
        seed=SEED
    )

    y_train_scenario = y[idx_train_scenario]

    n_total_train = len(idx_train_scenario)
    n_pos_train = int(np.sum(y_train_scenario == 1))
    n_neg_train = int(np.sum(y_train_scenario == 0))

    train_dist_rows.append({
        "Scenario": scenario_name,
        "Target_Ratio_Negatif_per_Positif": "original" if ratio is None else f"{ratio}:1",
        "Train_Total": n_total_train,
        "Train_Tidak_Hujan_Lebat": n_neg_train,
        "Train_Hujan_Lebat": n_pos_train,
        "Train_Persen_Hujan_Lebat": round(n_pos_train / n_total_train * 100, 2),
    })

    X_seq_train = X_seq_scaled[idx_train_scenario]
    X_aux_train = X_aux_scaled[idx_train_scenario]
    y_train = y[idx_train_scenario]
...
train_dist_table = pd.DataFrame(train_dist_rows)
train_dist_table.to_csv(
    f"{OUT_TABLES}/tabel_oversampling_03_distribusi_train_skenario.csv",
    index=False
)
...
eval_table.to_csv(
    f"{OUT_TABLES}/tabel_oversampling_04_evaluasi_cnn.csv",
    index=False
)

print("\nDistribusi train per skenario:")
print(train_dist_table)

print("\nEvaluasi CNN random oversampling:")
print(eval_table)


# ============================================================
# 7. PERBANDINGAN DENGAN BASELINE PADA TEST
# ============================================================
...
cnn_comp["Tipe"] = "CNN_RandomOversampling"
...
comparison_table.to_csv(
    f"{OUT_TABLES}/tabel_oversampling_05_perbandingan_test.csv",
    index=False
)


# ============================================================
# 8. PILIH MODEL UTAMA BERDASARKAN VALIDASI
# ============================================================

best_scenario = eval_table.loc[0, "Scenario"]
best_row = eval_table.iloc[0].to_dict()

with open(f"{OUT_PROCESSED}/selected_random_oversampling_by_validation.json", "w", encoding="utf-8") as f:
...
cm_table.to_csv(
    f"{OUT_TABLES}/tabel_oversampling_06_confusion_matrix_best.csv",
    index=False
)


# ============================================================
# 9. GAMBAR LOSS, ROC, DAN CONFUSION MATRIX MODEL TERPILIH
# ============================================================
...
plt.title(f"Kurva Loss CNN Sequence 14 Hari ({best_scenario})")
plt.xlabel("Epoch")
plt.ylabel("Binary Cross-Entropy Loss")
plt.legend()
plt.tight_layout()
plt.savefig(f"{OUT_FIGURES}/gambar_oversampling_01_loss_best.png", dpi=300)
plt.close()
...
if len(np.unique(y_true_best)) == 2:
    ...
    plt.title(f"Kurva ROC CNN Sequence 14 Hari pada Data Uji ({best_scenario})")
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{OUT_FIGURES}/gambar_oversampling_02_roc_best.png", dpi=300)
    plt.close()
...
ax.set_title(f"Confusion Matrix CNN Sequence 14 Hari pada Data Uji ({best_scenario})")
ax.set_xlabel("Kelas Prediksi")
ax.set_ylabel("Kelas Aktual")

plt.tight_layout()
plt.savefig(
    f"{OUT_FIGURES}/gambar_oversampling_03_confusion_matrix_best.png",
    dpi=300,
    bbox_inches="tight"
)
plt.close()
...
print("\n============================================================")
print("SELESAI CNN SEQUENCE 14 HARI + RANDOM OVERSAMPLING TRAIN")
print("============================================================")
