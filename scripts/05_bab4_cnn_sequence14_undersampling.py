...
OUT_BASE = "outputs/cnn_sequence14_undersampling"
...
# Rasio kelas mayoritas terhadap kelas minoritas pada TRAIN.
# original = tanpa undersampling.
UNDERSAMPLING_SCENARIOS = {
    "original": None,
    "us_1_1": 1,
    "us_1_3": 3,
    "us_1_5": 5,
...
def build_sequence_aux_cnn(seq_shape, aux_shape):
    ...
    model = models.Model(
        ...
        outputs=output,
        name="cnn_sequence14_aux_undersampling"
    )
...
def sample_train_indices_by_month(df_model, ratio, seed=42):
    """
    Undersampling hanya pada data train.
    Semua kelas positif dipertahankan.
    Kelas negatif diambil secara proporsional berdasarkan bulan target.
    ratio = jumlah negatif / jumlah positif.
    """

    df_train = df_model[df_model["Split"] == "train"].copy()

    pos = df_train[df_train["Label_Lebat_50"] == 1].copy()
    neg = df_train[df_train["Label_Lebat_50"] == 0].copy()

    if ratio is None:
        return df_train.index.to_numpy()

    n_pos = len(pos)
    n_neg_needed = int(n_pos * ratio)
    n_neg_needed = min(n_neg_needed, len(neg))

    month_counts = neg["Target_Month"].value_counts().sort_index()
    quotas = month_counts / month_counts.sum() * n_neg_needed

    floors = np.floor(quotas).astype(int)
    remaining = n_neg_needed - floors.sum()

    fractions = (quotas - floors).sort_values(ascending=False)

    for month in fractions.index[:remaining]:
        floors.loc[month] += 1

    sampled_neg_indices = []

    for month, n_sample in floors.items():
        if n_sample <= 0:
            continue

        group = neg[neg["Target_Month"] == month]
        n_sample = min(n_sample, len(group))

        sampled = group.sample(
            n=n_sample,
            random_state=seed + int(month)
        )

        sampled_neg_indices.extend(sampled.index.tolist())

    # Jika karena pembulatan masih kurang, ambil sisa dari negatif yang belum terpilih
    if len(sampled_neg_indices) < n_neg_needed:
        remaining_needed = n_neg_needed - len(sampled_neg_indices)

        remaining_pool = neg[~neg.index.isin(sampled_neg_indices)]
        ...
        sampled_neg_indices.extend(extra.index.tolist())

    final_indices = np.array(pos.index.tolist() + sampled_neg_indices)
...
dist_table.to_csv(
    f"{OUT_TABLES}/tabel_undersampling_01_distribusi_label_asli.csv",
    index=False
)

df_model.to_csv(
    f"{OUT_PROCESSED}/dataset_cnn_sequence14_undersampling_metadata.csv",
    index=False
)
...
# Scaler tetap dipelajari dari seluruh data train asli, bukan dari validation/test.
# Ini menjaga normalisasi stabil, tetapi supervised learning tetap memakai undersampled train.
...
baseline_table.to_csv(
    f"{OUT_TABLES}/tabel_undersampling_02_evaluasi_baseline.csv",
    index=False
)


# ============================================================
# 6. TRAIN CNN DENGAN SKENARIO UNDERSAMPLING
# ============================================================
...
for scenario_name, ratio in UNDERSAMPLING_SCENARIOS.items():
    print(f"\nTraining skenario: {scenario_name}")

    idx_train_scenario = sample_train_indices_by_month(
        df_model=df_model,
        ratio=ratio,
        seed=SEED
    )

    df_train_scenario = df_model.loc[idx_train_scenario].copy()

    n_total_train = len(df_train_scenario)
    n_pos_train = int(df_train_scenario["Label_Lebat_50"].sum())
    n_neg_train = n_total_train - n_pos_train

    train_dist_rows.append({
        "Scenario": scenario_name,
        "Ratio_Negatif_per_Positif": "original" if ratio is None else f"1:{ratio}",
...
train_dist_table.to_csv(
    f"{OUT_TABLES}/tabel_undersampling_03_distribusi_train_skenario.csv",
    index=False
)
...
eval_table.to_csv(
    f"{OUT_TABLES}/tabel_undersampling_04_evaluasi_cnn.csv",
    index=False
)
...
cnn_comp["Tipe"] = "CNN_Undersampling"
...
comparison_table.to_csv(
    f"{OUT_TABLES}/tabel_undersampling_05_perbandingan_test.csv",
    index=False
)
...
with open(f"{OUT_PROCESSED}/selected_undersampling_by_validation.json", "w", encoding="utf-8") as f:
...
cm_table.to_csv(
    f"{OUT_TABLES}/tabel_undersampling_06_confusion_matrix_best.csv",
    index=False
)
...
plt.legend()
plt.tight_layout()
plt.savefig(f"{OUT_FIGURES}/gambar_undersampling_01_loss_best.png", dpi=300)
plt.close()
...
if len(np.unique(y_true_best)) == 2:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{OUT_FIGURES}/gambar_undersampling_02_roc_best.png", dpi=300)
    plt.close()
...
plt.tight_layout()
plt.savefig(
    f"{OUT_FIGURES}/gambar_undersampling_03_confusion_matrix_best.png",
    dpi=300,
    bbox_inches="tight"
)
plt.close()
...
print("\n============================================================")
print("SELESAI CNN SEQUENCE 14 HARI + UNDERSAMPLING TRAIN")
print("============================================================")
