...
OUT_BASE = "outputs/cnn_feature_engineering_multihorizon"
...
# Untuk tahap pertama: tanpa class_weight dulu.
# Kalau ada horizon yang membaik, baru kita uji class_weight/focal loss.
...
def build_sequence_aux_cnn(seq_shape, aux_shape):
    ...
    model = models.Model(
        ...
        outputs=output,
        name="cnn_feature_engineering_multihorizon"
    )
...
def evaluate_baseline_horizon(feature_name, threshold, split_name, split_df, label_col):
...
# Target date utama tetap t+1 sebagai awal horizon.
df["Target_Date"] = df["Tanggal_Observasi"].shift(-1)

# ============================================================
# 2. TARGET MULTI-HORIZON
# ============================================================

for h in HORIZONS:
    future_cols = []
    for k in range(1, h + 1):
        col = f"Rainfall_t_plus_{k}"
        df[col] = df["Rainfall"].shift(-k)
        future_cols.append(col)

    df[f"Rainfall_Target_Max_H{h}"] = df[future_cols].max(axis=1)
    df[f"Label_Lebat_H{h}"] = (
        df[f"Rainfall_Target_Max_H{h}"] >= HEAVY_RAIN_THRESHOLD
    ).astype(int)

# Hapus baris akhir yang tidak punya target lengkap untuk horizon terbesar.
df = df.dropna(subset=["Target_Date", f"Rainfall_t_plus_{max(HORIZONS)}"]).copy()
...
# Untuk metadata kategori target H1
df["Rainfall_Target_H1"] = df["Rainfall_t_plus_1"]
df["Kategori_Target_H1"] = np.select(
    [
        df["Rainfall_Target_H1"] > 150,
        (df["Rainfall_Target_H1"] >= 100) & (df["Rainfall_Target_H1"] <= 150),
        (df["Rainfall_Target_H1"] >= 50) & (df["Rainfall_Target_H1"] < 100),
    ],
    [
        "Ekstrem >150 mm",
...
# ============================================================
# 3. ANOMALY DASARIAN BERDASARKAN TRAIN SAJA
# ============================================================
...
# ============================================================
# 6. BENTUK SEQUENCE 14 HARI DENGAN 2 CHANNEL
# ============================================================
...
for i in range(RAIN_SEQUENCE_DAYS - 1, len(df)):
    ...
    seq = np.stack([rain_seq, anom_seq], axis=1)
    ...
    for h in HORIZONS:
        y_by_horizon[h].append(int(current_row[f"Label_Lebat_H{h}"]))
...
y_by_horizon = {
    h: np.array(y_by_horizon[h], dtype=int)
    for h in HORIZONS
}
...
# ============================================================
# 7. SIMPAN DISTRIBUSI LABEL MULTI-HORIZON DAN DAFTAR FITUR
# ============================================================

dist_rows = []

for h in HORIZONS:
    label_col = f"Label_Lebat_H{h}"
    ...
    for split_name, split_df in df_model.groupby("Split"):
        ...
        dist_rows.append({
            ...
            "Split": split_name,
            "Jumlah_Data": n_total,
            "Tidak_Ada_Hujan_Lebat": n_neg,
            "Ada_Hujan_Lebat_Dalam_Horizon": n_pos,
            "Persentase_Positive": round(n_pos / n_total * 100, 2),
        })

dist_table = pd.DataFrame(dist_rows)
dist_table.to_csv(
    f"{OUT_TABLES}/tabel_mh_01_distribusi_label_multihorizon.csv",
    index=False
)
...
feature_table.to_csv(
    f"{OUT_TABLES}/tabel_mh_02_daftar_fitur.csv",
    index=False
)

df_model.to_csv(
    f"{OUT_PROCESSED}/dataset_cnn_feature_engineering_multihorizon_metadata.csv",
    index=False
)
...
for h in HORIZONS:
    ...
    # Baseline tetap aturan R1/R3/R5/R7/R10 >=50, dievaluasi terhadap target horizon.
    ...
    for scenario_name, class_weight in CLASS_WEIGHT_SCENARIOS.items():
        print(f"Training skenario: H{h}_{scenario_name}")
        ...
        key = f"H{h}_{scenario_name}"
        history_store[key] = history.history
        threshold_scan_store[key] = threshold_scan

        pred_df = df_test[[
            "Tanggal_Observasi",
            "Target_Date",
            f"Rainfall_Target_Max_H{h}",
            label_col
        ...
        pred_df["Horizon_Hari"] = h
        pred_df["Scenario"] = scenario_name

        prediction_store[key] = pred_df

        threshold_scan.to_csv(
            f"{OUT_PROCESSED}/threshold_scan_validation_H{h}_{scenario_name}.csv",
            index=False
        )

        pred_df.to_csv(
            f"{OUT_PROCESSED}/predictions_test_H{h}_{scenario_name}.csv",
            index=False
        )
...
baseline_table.to_csv(
    f"{OUT_TABLES}/tabel_mh_03_evaluasi_baseline.csv",
    index=False
)
...
eval_table = eval_table.sort_values(
    ["Horizon_Hari", "Val_CSI", "Val_AUROC", "Val_FAR"],
    ascending=[True, False, False, True]
).reset_index(drop=True)

eval_table.to_csv(
    f"{OUT_TABLES}/tabel_mh_04_evaluasi_cnn_multihorizon.csv",
    index=False
)

print("\nEvaluasi CNN multi-horizon:")
print(eval_table)


# ============================================================
# 10. PERBANDINGAN TEST RINGKAS
# ============================================================
...
for _, row in eval_table.iterrows():
    comparison_rows.append({
        "Horizon_Hari": row["Horizon_Hari"],
        "Model": f"CNN_FE_H{row['Horizon_Hari']}_{row['Scenario']}",
        "POD": row["Test_POD"],
        "FAR": row["Test_FAR"],
        "CSI": row["Test_CSI"],
        ...
        "FN": row["Test_FN"],
        "TN": row["Test_TN"],
        "Tipe": "CNN_FeatureEngineering_MultiHorizon"
    })

# Tambahkan baseline terbaik per horizon berdasarkan CSI test
baseline_test = baseline_table[baseline_table["Split"] == "test"].copy()

for h in HORIZONS:
    sub = baseline_test[baseline_test["Horizon_Hari"] == h].copy()
    if len(sub) == 0:
        continue
    best_base = sub.sort_values(["CSI", "POD"], ascending=[False, False]).iloc[0]

    comparison_rows.append({
        "Horizon_Hari": h,
        "Model": best_base["Model"],
        "POD": best_base["POD"],
        "FAR": best_base["FAR"],
        "CSI": best_base["CSI"],
        "AUROC": best_base["AUROC"],
        "TP": best_base["TP"],
        "FP": best_base["FP"],
        "FN": best_base["FN"],
        "TN": best_base["TN"],
...
comparison_table = comparison_table.sort_values(
    ["Horizon_Hari", "CSI", "POD"],
    ascending=[True, False, False]
).reset_index(drop=True)

comparison_table.to_csv(
    f"{OUT_TABLES}/tabel_mh_05_perbandingan_test_multihorizon.csv",
    index=False
)


# ============================================================
# 11. PILIH MODEL TERBAIK SECARA SUBSTANTIF
# ============================================================

# Untuk eksplorasi, pilih berdasarkan Test_CSI tertinggi.
# Untuk penulisan resmi, tetap hati-hati: jangan pilih dari test tanpa dijelaskan sebagai analisis eksploratif.
best_test_row = eval_table.sort_values(
    ["Test_CSI", "Test_AUROC", "Test_POD"],
    ascending=[False, False, False]
).iloc[0].to_dict()

best_h = int(best_test_row["Horizon_Hari"])
best_scenario = best_test_row["Scenario"]
best_key = f"H{best_h}_{best_scenario}"

with open(f"{OUT_PROCESSED}/selected_best_multihorizon_by_test_for_analysis.json", "w", encoding="utf-8") as f:
    json.dump(best_test_row, f, indent=4, ensure_ascii=False)

best_pred_df = prediction_store[best_key]
...
cm_table = pd.DataFrame({
    ...
    "Jumlah": [tn, fp, fn, tp],
    "Keterangan": [
        "Tidak ada hujan lebat dalam horizon diprediksi benar",
        "False alarm: diprediksi ada hujan lebat dalam horizon, tetapi tidak terjadi",
        "Kejadian hujan lebat dalam horizon tidak terdeteksi",
        "Kejadian hujan lebat dalam horizon berhasil terdeteksi"
    ]
})

cm_table.to_csv(
    f"{OUT_TABLES}/tabel_mh_06_confusion_matrix_best.csv",
    index=False
)


# ============================================================
# 12. GAMBAR MODEL TERBAIK ANALISIS
# ============================================================

hist = history_store[best_key]

plt.figure(figsize=(9, 5))
plt.plot(hist["loss"], label="Training loss")
plt.plot(hist["val_loss"], label="Validation loss")
plt.title(f"Kurva Loss CNN FE Multi-Horizon H{best_h} ({best_scenario})")
plt.xlabel("Epoch")
plt.ylabel("Binary Cross-Entropy Loss")
plt.legend()
plt.tight_layout()
plt.savefig(f"{OUT_FIGURES}/gambar_mh_01_loss_best.png", dpi=300)
plt.close()
...
if len(np.unique(y_true_best)) == 2:
    ...
    plt.title(f"Kurva ROC CNN FE Multi-Horizon H{best_h} pada Data Uji")
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{OUT_FIGURES}/gambar_mh_02_roc_best.png", dpi=300)
    plt.close()
...
ax.set_xticklabels(["Prediksi 0\nTidak Ada Kejadian", "Prediksi 1\nAda Kejadian"])

ax.set_yticks([0.5, 1.5])
ax.set_yticklabels(["Aktual 0\nTidak Ada Kejadian", "Aktual 1\nAda Kejadian"])

ax.set_title(f"Confusion Matrix CNN FE Multi-Horizon H{best_h} pada Data Uji")
ax.set_xlabel("Kelas Prediksi")
ax.set_ylabel("Kelas Aktual")

plt.tight_layout()
plt.savefig(
    f"{OUT_FIGURES}/gambar_mh_03_confusion_matrix_best.png",
    dpi=300,
    bbox_inches="tight"
)
plt.close()
...
print("\n============================================================")
print("SELESAI CNN FEATURE ENGINEERING MULTI-HORIZON")
print("============================================================")

print("\nDistribusi label multi-horizon:")
print(dist_table)

print("\nEvaluasi CNN multi-horizon:")
print(eval_table)

print("\nPerbandingan test multi-horizon:")
print(comparison_table)

print("\nModel terbaik berdasarkan Test_CSI untuk analisis:")
print(best_test_row)

print("\nConfusion matrix model terbaik analisis:")
print(cm_table)
